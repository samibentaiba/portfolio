        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          {branches.map((branch) => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 sm:py-1.5 rounded text-xs font-medium transition-all ${
                isMobile ? "min-h-[44px]" : ""
              } ${
            ...
            >
              <span
                ...
                style={{
                  ...
                  boxShadow: visibleBranches.has(branch.id)
                    ? `0 0 6px ${branch.color}`
                    : "none",
                }}
                aria-hidden="true"
              />
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          {(["small", "medium", "large"] as const).map((size) => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 sm:py-1.5 rounded text-xs font-medium transition-all capitalize ${
                isMobile ? "min-h-[44px]" : ""
              } ${
                sizeFilter.has(size)
                  ? "bg-primary/10 text-foreground border border-primary/20"
...
const TimelineVisualization = memo(function TimelineVisualization({
  ...
  // Calculate positions
  const { svgWidth, svgHeight, branchYPositions, pointPositions } =
    useMemo(() => {
      ...
      const pPositions: Record<
        string,
        { x: number; y: number; branch: string; color: string }
      > = {};
      adjustedBranches.forEach((branch) => {
  ...
  // Render branch lines
  const renderBranchLines = useCallback(
    (branch: CareerBranch) => {
        ...
        point.linkedTo?.forEach((link) => {
          ...
          if (dy > 0) {
            ...
            pathD = `M ${fromPos.x} ${fromPos.y} 
                     ...
                     Q ${fromPos.x} ${midY} ${
              fromPos.x + (dx > 0 ? cornerSize : -cornerSize)
            } ${midY}
          ...
          lines.push(
            <path
              ...
              style={{
                filter: isHovered
                  ? `drop-shadow(0 0 8px ${branch.color})`
                  : "none",
  ...
  const renderPoints = useCallback(
    (branch: CareerBranch) => {
      ...
      return branch.points.map((point) => {
        ...
        const isHovered =
          hoveredPoint === point.id || clickedPoint === point.id;
  ...
  return (
    <div
      className={`flex flex-col ${
        isMobile ? "gap-4" : "sm:flex-row"
      } flex-wrap gap-4 sm:gap-6 mt-6 justify-center text-sm text-muted-foreground`}
    >
      ...
      <div
        className={`${
          isMobile ? "border-t pt-3" : "border-l pl-6"
        } border-border flex gap-3 sm:gap-4 flex-wrap justify-center`}
      >
        {(["small", "medium", "large"] as const).map((size) => (
...
const CareerTimeline = memo(function CareerTimeline({
  ...
  const assignUniqueXPositions = useCallback(
    (inputBranches: CareerBranch[]) => {
  ...
  const adjustedBranches = useMemo(() => {
    return assignUniqueXPositions(branches)
      ...
      .map((branch) => ({
        ...branch,
        points: branch.points.filter((point) =>
          sizeFilter.has(point.milestone.size)
        ),
      }));
